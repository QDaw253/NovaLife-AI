...
def build_goal_plan_prompt(goal_data):
    ...
    deadline_rule = (
        "Deadline của milestone và task phải từ ngày hiện tại trở đi "
        "và không được vượt quá deadline của mục tiêu."
        if goal_deadline
        else
...
5. Tất cả deadline phải bằng hoặc sau ngày hiện tại: {today}.
   TUYỆT ĐỐI KHÔNG được tạo bất kỳ deadline nào trước {today}.
   Kế hoạch phải bắt đầu từ ngày hiện tại trở đi.

6. Deadline của mỗi task không được vượt quá deadline
   của milestone chứa task đó.

7. Nếu mục tiêu có deadline, deadline của tất cả milestone
   và task không được vượt quá deadline của mục tiêu.

8. Nếu mục tiêu không có deadline, hãy tự xây dựng timeline
   hợp lý bắt đầu từ ngày hiện tại.

9. Thứ tự của milestone và task phải bắt đầu từ 1
   và tăng dần liên tục.
...
19. Không thêm giải thích, lời chào hoặc bất kỳ nội dung nào
    bên ngoài JSON.
