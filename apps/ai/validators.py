...
class GoalAITaskResultSerializer(serializers.Serializer):
    ...
    def validate_deadline(self, value):
        if value < date.today():
            raise serializers.ValidationError("Deadline của task không được nằm trong quá khứ.")
...
class GoalAIMilestoneResultSerializer(serializers.Serializer):
    ...
    deadline = serializers.DateField()
    order = serializers.IntegerField(min_value=1,)
    tasks = GoalAITaskResultSerializer( many=True,)
    def validate_deadline(self, value):
        if value < date.today():
...
class GoalAIPlanResultSerializer(serializers.Serializer):
    ...
    estimated_duration = serializers.CharField( max_length=100,)
    recommended_hours_per_week = serializers.IntegerField(
        min_value=1,
        allow_null=True,
        required=False,
    )
    milestones = GoalAIMilestoneResultSerializer( many=True,)
    ...
    def validate(self, attrs):
        goal_deadline = self.context.get("goal_deadline")
