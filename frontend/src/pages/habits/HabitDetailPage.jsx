...
import { completeHabitToday, getHabit } from '../../api/habits'
...
function HabitDetailPage() {
  ...
  return (
    <div>
      ...
      <p>{habit.description || 'Không có mô tả'}</p>
      ...
      <p>Ngày bắt đầu: {habit.start_date}</p>
      <p>Ngày kết thúc: {habit.end_date || 'Không có'}</p>
      ...
      {habit.recent_logs?.length > 0 ? (
        habit.recent_logs.map((log) => (
          <div key={log.id}>
            <p>Ngày: {log.date}</p>
            <p>Giá trị: {log.value}</p>
            <p>Trạng thái: {log.status}</p>
            <p>Ghi chú: {log.note || 'Không có'}</p>

            <hr />
          </div>
        ))
      ) : (
        <p>Chưa có lịch sử thực hiện.</p>
      )}
    </div>
  )
}
