...
import { getClothingItem, updateClothingItem } from '../../api/wardrobe'
...
function EditWardrobePage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(responseData?.message || 'Không thể lưu thay đổi. Vui lòng kiểm tra lại thông tin.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <div className="wardrobe-edit-page">
      <header className="wardrobe-edit-header">
        <button type="button" className="wardrobe-edit-back" onClick={() => navigate(`/wardrobe/${id}`)}>
          ← Quay lại chi tiết
        </button>
        <p className="wardrobe-edit-eyebrow">CHỈNH SỬA</p>
        <h1>Chỉnh sửa trang phục</h1>
        <p>Cập nhật thông tin để tủ đồ NovaLife luôn phản ánh chính xác những món đồ bạn đang sở hữu.</p>
      </header>
      ...
      <div className="wardrobe-edit-layout">
        <form className="wardrobe-edit-form" onSubmit={handleSubmit}>
          ...
          <section className="wardrobe-edit-section">
            ...
            <div className="wardrobe-edit-fields">
              <div className={`wardrobe-edit-field wardrobe-edit-field-full ${fieldErrors.name ? 'has-error' : ''}`}>
                ...
                <input id="name" name="name" type="text" value={formData.name} onChange={handleChange} placeholder="Ví dụ: Áo thun trắng" required />
          ...
          {error && <div className="wardrobe-edit-error">{error}</div>}

          <div className="wardrobe-edit-actions">
            <button type="button" className="wardrobe-edit-cancel" onClick={() => navigate(`/wardrobe/${id}`)} disabled={saving}>
              Hủy
            </button>
            <button type="submit" className="wardrobe-edit-submit" disabled={saving}>
        ...
        <aside className="wardrobe-edit-guide">
          ...
          <p>Thông tin trang phục càng chính xác thì các module AI sử dụng tủ đồ sau này càng có dữ liệu tốt hơn.</p>
